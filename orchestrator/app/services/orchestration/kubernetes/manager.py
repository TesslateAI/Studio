...
class KubernetesContainerManager:
    ...
    async def _cleanup_s3_mode(self, idle_timeout_minutes: int) -> list[str]:
        ...
        try:
            ...
            for k8s_env in k8s_environments:
                ...
                if idle_time > idle_timeout_seconds:
                    logger.info(
                        f"[K8S:CLEANUP:S3] Hibernating {project_key} (idle for {idle_minutes:.1f} minutes)"
                    )
                    ...
                    logger.debug(
                        f"[K8S:CLEANUP:S3] {project_key} is active (idle for {idle_minutes:.1f} minutes)"
                    )
        ...
        logger.info(
            f"[K8S:CLEANUP:S3] ✅ Cleanup completed: Hibernated {len(hibernated)} environments"
        )
        return hibernated
    ...
    async def _cleanup_persistent_mode(self, idle_timeout_minutes: int) -> list[str]:
        ...
        logger.info(
            f"[K8S:CLEANUP:PERSISTENT] Tier 1: Scale to 0 after {idle_timeout_minutes} minutes idle"
        )
        logger.info(
            "[K8S:CLEANUP:PERSISTENT] Tier 2: Delete resources after 24 hours at 0 replicas"
        )
            ...
            for project_key in list(self.paused_at_tracker.keys()):
                ...
                if paused_duration > tier2_timeout_seconds:
                    ...
                    if env_info:
                        ...
                        logger.info(
                            f"[K8S:CLEANUP:TIER2] Deleting long-paused environment {project_key} (paused for {paused_hours:.1f} hours)"
                        )
                        ...
                        try:
                            ...
                            logger.info(
                                f"[K8S:CLEANUP:TIER2] ✅ Deleted resources for {project_key}"
                            )
                        except Exception as e:
                            logger.error(
                                f"[K8S:CLEANUP:TIER2] ❌ Failed to delete {project_key}: {e}"
                            )
                    else:
            ...
            for k8s_env in k8s_environments:
                ...
                if replicas == 0:
                    if project_key not in self.paused_at_tracker:
                        ...
                        logger.info(
                            f"[K8S:CLEANUP:TIER1] Tracking already-paused environment {project_key}"
                        )
                    continue
                ...
                if idle_time > tier1_timeout_seconds:
                    logger.info(
                        f"[K8S:CLEANUP:TIER1] Scaling down idle environment {project_key} (idle for {idle_minutes:.1f} minutes)"
                    )
                    try:
                        ...
                        logger.error(
                            f"[K8S:CLEANUP:TIER1] ❌ Failed to scale down {project_key}: {e}"
                        )
                else:
                    logger.debug(
                        f"[K8S:CLEANUP:TIER1] {project_key} is active (idle for {idle_minutes:.1f} minutes)"
                    )

        except Exception as e:
            logger.error(f"[K8S:CLEANUP] ❌ Unexpected error during cleanup: {e}")

        len(scaled_down) + len(removed)
        logger.info(
            f"[K8S:CLEANUP] ✅ Cleanup completed: {len(scaled_down)} scaled down, {len(removed)} deleted"
        )
