...
- `local.py` — filesystem + subprocess backend. `_get_project_root(project)`
  returns `$TESSLATE_STUDIO_HOME/projects/{slug}-{id}` under desktop mode,
  else falls back to `$PROJECT_ROOT` / `cwd`.
- `local_ports.py` — per-project host-port allocator for the local runtime.
  Persists to `$TESSLATE_STUDIO_HOME/cache/ports.json` (atomic write).
...
- `app/services/desktop_paths.py` — `$TESSLATE_STUDIO_HOME` resolver.
