"""Seed the Tesslate "CRM with Postgres" matrix demo app.
...
Requires one of:
  - TSL_APPS_DEV_AUTO_APPROVE=1 (preferred)
  - TSL_APPS_SKIP_APPROVAL=1    (legacy)

This app declares a `${secret:pg-creds/password}` reference. The secret
itself is per-install (lives in the project namespace, not in `tesslate`),
so it must be created AFTER install — see seeds/apps/crm-with-postgres/README.md.
"""
...
async def main() -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        proj = Project(
            id=uuid.uuid4(),
            name="Tesslate CRM with Postgres (source)",
            slug=f"crm-with-postgres-src-{uuid.uuid4().hex[:6]}",
            owner_id=creator.id,
            team_id=team_id,
            visibility="team",
            volume_id=volume_id,
