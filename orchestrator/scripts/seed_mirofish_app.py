"""Seed the MiroFish Tesslate App via the federated marketplace.

Image-based: container pulls upstream ``ghcr.io/666ghj/mirofish:latest``.
The bundle we publish contains only the manifest + .tesslate/config.json.
...
Run inside the backend pod (the seed_apps cron does this automatically):
    ...
    kubectl --context=tesslate -n tesslate exec deploy/tesslate-backend -- \\
      python -m scripts.seed_mirofish_app
...
async def main() -> int:
    ...
    name = str(app_meta.get("name") or "MiroFish")
