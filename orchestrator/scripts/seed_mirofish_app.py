"""Seed the MiroFish (swarm-intelligence prediction engine) Tesslate App.

Uses the upstream-published image `ghcr.io/666ghj/mirofish:latest` directly —
no local Dockerfile. The image must already be loaded into minikube's node
cache (see seeds/apps/mirofish/README.md).

Run inside the backend pod:
    kubectl --context=tesslate -n tesslate exec deploy/tesslate-backend -- \
      env TSL_APPS_DEV_AUTO_APPROVE=1 python -m scripts.seed_mirofish_app
"""
...
logger = logging.getLogger("seed_mirofish")

SLUG = "mirofish"
_SEEDS_SLUG = "mirofish"
...
async def main() -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        proj = Project(
            id=uuid.uuid4(),
            name="MiroFish (source)",
            slug=f"mirofish-src-{uuid.uuid4().hex[:6]}",
            owner_id=creator.id,
            team_id=team_id,
            visibility="team",
            volume_id=volume_id,
