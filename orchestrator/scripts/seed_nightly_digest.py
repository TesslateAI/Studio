"""Seed the Nightly Activity Digest headless Tesslate App via federated marketplace.

Headless cron-triggered digest. Requires a cluster secret
``llama-api-credentials`` in the ``tesslate`` namespace with an ``api_key``
entry — same as ``seed_crm_app``.
...
async def main() -> int:
    ...
    name = str(app_meta.get("name") or "Nightly Activity Digest")
