"""Seed the Law Onboarding & Redline demo app into the marketplace.

Bundle-based: the seeder creates a fresh Hub volume, streams the asset tree
from ``seeds/apps/law-onboarding/`` into it, creates an ``app_source`` Project
...
The app runs in "demo mode" (see ``lib/demo-data.ts``) when DATABASE_URL is
unset, so no Postgres service container is required.

Run inside the backend pod:
    kubectl --context=tesslate-beta-eks -n tesslate exec deploy/tesslate-backend -- \
      env TSL_APPS_DEV_AUTO_APPROVE=1 python -m scripts.seed_law_onboarding_app
"""
...
async def main() -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        proj = Project(
            id=uuid.uuid4(),
            name="Law Onboarding (source)",
            slug=f"law-onboarding-src-{uuid.uuid4().hex[:6]}",
            owner_id=creator.id,
            team_id=team_id,
            visibility="team",
            volume_id=volume_id,
