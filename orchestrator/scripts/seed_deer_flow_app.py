"""Seed the DeerFlow Tesslate App via the federated marketplace.

Image-based: container pulls ``tesslate-deerflow:latest``. The bundle we
publish contains only the manifest + .tesslate/config.json so
...
Run inside the backend pod (the seed_apps cron does this automatically):
    ...
    kubectl --context=tesslate -n tesslate exec deploy/tesslate-backend -- \\
      python -m scripts.seed_deer_flow_app
...
async def main() -> int:
    ...
    name = str(app_meta.get("name") or "DeerFlow")
