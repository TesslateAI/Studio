"""Seed the Tesslate CRM demo app into the marketplace.
...
Requires one of:
  - TSL_APPS_DEV_AUTO_APPROVE=1 (preferred, new flag)
  - TSL_APPS_SKIP_APPROVAL=1    (legacy flag, still honored)

Also requires a cluster secret `llama-api-credentials` in the `tesslate`
...
async def _write_file(fops: FileOpsClient, volume_id: str, rel_path: str, data: bytes) -> None:
    # Prefer write_file_safe (creates parent dirs); fall back to write_file.
...
async def main() -> int:
    ...
    async with AsyncSessionLocal() as db:
        ...
        proj = Project(
            id=uuid.uuid4(),
            name="Tesslate CRM (source)",
            slug=f"crm-demo-src-{uuid.uuid4().hex[:6]}",
            owner_id=creator.id,
            team_id=team_id,
            visibility="team",
            volume_id=volume_id,
