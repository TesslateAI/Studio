...
from app.k8s_container_manager import (  # noqa: E402
    KubernetesContainerManager as DevContainerManager,
)
    ...
    async def test_multi_user_system(self):
        ...
        try:
            ...
            for scenario in test_scenarios:
                ...
                try:
                    port = await self.manager.start_container(
                        self.test_project_path, scenario["project_id"], scenario["user_id"]
                    )
                    started_containers.append({**scenario, "port": port})
            ...
            for container in all_containers:
                self.log(
                    f"Container: {container.get('container_name')} - User {container.get('user_id')}, Project {container.get('project_id')}, Port {container.get('port')}"
                )
            ...
            for container in started_containers:
                ...
                self.log(
                    f"{container['name']}: URL={url}, Status={status.get('status', 'unknown')}"
                )
