...
class ContainerSystemTest:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log(
                        f"[OK] Response contains HTML: {'<!DOCTYPE html>' in response.text or '<html' in response.text}"
                    )
                    return True
    ...
    def test_hot_reload(self):
        ...
        modified_content = original_content.replace(
            "Start building something amazing.", f"Start building something amazing. {test_marker}"
        )
