...
def test_get_returns_none_when_unpaired(opensail_home: Path) -> None:
...
def test_set_then_get_roundtrip(opensail_home: Path) -> None:
    from app.services import token_store
    ...
    path = opensail_home / "cache" / "cloud_token.json"
    assert path.is_file()
...
def test_env_var_overrides_file(opensail_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from app.services import token_store
...
def test_clear_removes_file(opensail_home: Path) -> None:
    from app.services import token_store
...
def test_set_rejects_empty(opensail_home: Path) -> None:
    from app.services import token_store
