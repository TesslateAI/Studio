...
| `commands.rs` | Tauri `invoke` surface exposed to the React frontend. | `get_api_url`, `get_bearer`, `get_user_token`, `get_cloud_token`, `clear_cloud_token`, `is_cloud_paired`, `open_in_ide`, `open_external_url`, `start_dragging`, `minimize_window`, `toggle_maximize_window`, `close_window` |
