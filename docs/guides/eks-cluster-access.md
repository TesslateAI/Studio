...
| `./scripts/aws-deploy.sh ...` fails with "Can you assume role ...eks-deployer?" | That script hardcodes `eks-deployer`, which only `eks_admin_iam_arns` members can assume. | If you need the script, have someone in `eks_admin_iam_arns` run it. For log reads and ad-hoc debugging, use the team role directly (above) — don't use the script. |
