...
| `./scripts/aws-deploy.sh ...` fails with "Can you assume role ...team-deployer?" | Your IAM user isn't in `tesslate-{env}-deployers` (or higher). | Ask an admin to add you: `aws iam add-user-to-group --user-name <you> --group-name tesslate-{env}-deployers`. To run the script against a different role (e.g. legacy `eks-deployer` for admin ops, or `team-observer` for dry runs), set `AWS_EKS_ROLE_ARN=arn:aws:iam::<AWS_ACCOUNT_ID>:role/...` before invoking. |
