...
export default function MarketplaceDetail() {
  ...
  const loadReviews = async () => {
    if (!item?.id || item.item_type !== 'agent') return;

    setLoadingReviews(true);
    try {
      const data = await marketplaceApi.getAgentReviews(item.id);
  ...
  useEffect(() => {
    if (item?.id && item.item_type === 'agent') {
      loadReviews();
    }
  }, [item?.id]);
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 py-12">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            {/* Description */}
            <p className={`text-lg mb-4 ${theme === 'light' ? 'text-black/70' : 'text-white/70'}`}>
              {item.description}
            </p>
